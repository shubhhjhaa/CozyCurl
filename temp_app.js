...
function animateCartIcon() {
  const navCartIcon = document.querySelector('.nav-icons a[onclick="navigate(\\'checkout\\')"]');
  if (navCartIcon) {
...
async function initProducts() {
  ...
  try {
    ...
    if (trendingContainer) trendingContainer.innerHTML = '<p style="text-align:center; width:100%; color:red;">Failed to load products. Please check connection.</p>';
    if (allContainer) allContainer.innerHTML = '<p style="text-align:center; width:100%; color:red;">Failed to load products. Please check connection.</p>';
  }
}
...
function addToCart(productId) {
  ...
  showStoreToast(product ? `${product.name} added successfully âœ¨` : 'Item added to your cart ðŸ›ï¸', 'success', '<button onclick="navigate(\'checkout\')" style="background: none; border: none; color: var(--primary); font-weight: 600; cursor: pointer; text-decoration: underline; font-size: 13px;">View</button>');
      ...
      <div class="cart-item">
        ...
        <div class="cart-item-details">
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 8px;">
            ...
            <span style="font-weight: 600;">â‚¹${itemTotal.toLocaleString('en-IN')}</span>
          </div>
        </div>
      </div>
    `;
...
function submitCheckout() {
  ...
  let orderText = `*New Order from ${name}* ðŸ›ï¸\n\n`;
  ...
  cart.forEach(item => {
    ...
    if (product) {
      const itemTotal = product.price * item.quantity;
      total += itemTotal;
      orderText += `- ${item.quantity}x ${product.name} (â‚¹${itemTotal.toLocaleString('en-IN')})\n`;
    }
  });
  ...
  showStoreToast(`Thanks ${name}! Redirecting to WhatsApp...`, 'success');
...
window.loadProductDetails = async function(id) {
  ...
  try {
    ...
    document.getElementById('pdp-price').innerText = `â‚¹${product.price.toLocaleString('en-IN')}`;
...
window.changePdpQty = function(delta) {
  ...
  document.getElementById('pdp-qty').innerText = currentPdpQty;
}
...
window.addPdpToCart = function() {
  ...
  renderCart();
  animateCartIcon();
  showStoreToast(`${currentPdpProduct.name} added to cart! ðŸ›ï¸`, 'success', '<button onclick="navigate(\'checkout\')" style="background: none; border: none; color: var(--primary); font-weight: 600; cursor: pointer; text-decoration: underline; font-size: 13px;">View</button>');
};
...
window.shareProduct = async function() {
  ...
  if (navigator.share) {
    ...
    navigator.clipboard.writeText(url).then(() => {
      showStoreToast("Product link copied ðŸ’–", 'success');
    });
  }
};
...
window.submitReview = async function(e) {
  ...
  try {
    ...
    showStoreToast('Thank you for your review! ðŸ’–', 'success');
    closeReviewModal();
